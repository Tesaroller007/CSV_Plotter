...
def reload_plot():
    ...
    # Ensure all subplot settings exist
    ...
    for r in range(rows):
        for c in range(cols):
            ...
            # Create secondary y-axis only if configured (or lazily if needed later)
            if subcfg.get('secondary', False):
            ...
            ax.grid(subcfg.get('grid', True))
            idx += 1

    # In der reload_plot Funktion, ersetzen Sie den Abschnitt für Achseneinstellungen mit:

    # Apply axis settings for each subplot
    for subplot_num, ax in axes.items():
        if ax:
            # Get subplot specific settings, fall back to global settings if not found
    ...
    for region in zoom_regions:
            ...
            ax_for_inset_parent = axes.get(region_subplot)
            if ax_for_inset_parent is None:
                print(f"Warning: zoom region references non-existing subplot {region_subplot}")
                continue

            # decide which axis to use for the inset plotting (primary or secondary)
            if y_axis_choice == 'secondary':
                ax_for_plot = secondary_axes.get(region_subplot)
                # if no secondary exists, fall back to primary and warn
                if ax_for_plot is None:
                    ax_for_plot = ax_for_inset_parent
            else:
                ax_for_plot = ax_for_inset_parent
            ...
            # create inset attached to the parent axes (for marking) but use ax_for_plot for plotting data
            axins = inset_axes(ax_for_inset_parent, width=width, height=height, loc=loc, borderpad=borderpad)

            # If plotting on secondary axis, replicate the y-scale by sharing x data and plotting on axins;
            # for simple behavior we plot on axins and not try to share y-axis between axins and parent secondary
            plotted_any = False
            for entry in entries:
                try:
                    ...
                    x = df.iloc[:, 0]
                    y = df.iloc[:, 1]
                    axins.plot(x, y, color=entry.get('color', '#000000'))
                    plotted_any = True
                except Exception as e:
                    print(f"Fehler beim Laden der Datei {entry.get('file_path','?')}: {e}")
                    continue

            # if nothing was plotted, leave inset empty but still set limits from region
            try:
            ...
            # mark inset on the parent axes (best-effort mapping)
            try:
                if region.get('loc') in ('upper right', 'lower left'):
                    mark_inset(ax_for_inset_parent, axins, loc1=2, loc2=4, fc="none", ec="0.5")
                elif region.get('loc') in ('upper left',):
                    mark_inset(ax_for_inset_parent, axins, loc1=1, loc2=3, fc="none", ec="0.5")
                elif region.get('loc') in ('lower right',):
                    mark_inset(ax_for_inset_parent, axins, loc1=3, loc2=1, fc="none", ec="0.5")
                elif region.get('loc') in ('center right',):
                    mark_inset(ax_for_inset_parent, axins, loc1=2, loc2=3, fc="none", ec="0.5")
                elif region.get('loc') in ('center left',):
                    mark_inset(ax_for_inset_parent, axins, loc1=1, loc2=4, fc="none", ec="0.5")
                elif region.get('loc') in ('lower center',):
                    mark_inset(ax_for_inset_parent, axins, loc1=1, loc2=2, fc="none", ec="0.5")
                elif region.get('loc') in ('upper center',):
                    mark_inset(ax_for_inset_parent, axins, loc1=3, loc2=4, fc="none", ec="0.5")
            except Exception:
                pass
...
def open_subplot_settings():
    ...
    def update_subplot_entries():
        ...
        for i in range(1, rows * cols + 1):
            ...
            ttk.Checkbutton(subplot_label, text="Show Grid", variable=grid_var).grid(row=3, column=0, sticky="w", padx=5, pady=2)

            legend_var = tk.BooleanVar(value=subcfg.get('legend', True))
            ttk.Checkbutton(subplot_label, text="Show Legend", variable=legend_var).grid(row=3, column=1, sticky="w", padx=5, pady=2)

            secondary_var = tk.BooleanVar(value=subcfg.get('secondary', False))
            ttk.Checkbutton(subplot_label, text="Secondary Y-axis", variable=secondary_var).grid(row=3, column=2, sticky="w", padx=5, pady=2)
...
def open_zoom_settings():
    ...
    def update_yaxis_menu():
        # show secondary option only if selected subplot has secondary y-axis enabled
        try:
            sel = int(subplot_var.get())
        except Exception:
            sel = 1
        has_secondary = subplot_settings.get('subplots', {}).get(sel, {}).get('secondary', False)
    ...
    tk.Label(zoom_window, text="Zoom X Max:").grid(row=2, column=0, sticky="w")
    x_max_entry = tk.Entry(zoom_window)
    x_max_entry.insert(0, str(zoom_x[1]))
    x_max_entry.grid(row=2, column=1)

    tk.Label(zoom_window, text="Zoom Y Min:").grid(row=3, column=0, sticky="w")
    y_min_entry = tk.Entry(zoom_window)
    y_min_entry.insert(0, str(zoom_y[0]))
    y_min_entry.grid(row=3, column=1)

    tk.Label(zoom_window, text="Zoom Y Max:").grid(row=4, column=0, sticky="w")
    y_max_entry = tk.Entry(zoom_window)
    y_max_entry.insert(0, str(zoom_y[1]))
    y_max_entry.grid(row=4, column=1)

    # Grid
    tk.Label(zoom_window, text="Show Grid in Zoom:").grid(row=5, column=0, sticky="w")
    zoom_grid_var = tk.BooleanVar(value=True)
    tk.Checkbutton(zoom_window, variable=zoom_grid_var).grid(row=5, column=1)

    # Ticks
    tk.Label(zoom_window, text="Show Ticks in Zoom:").grid(row=6, column=0, sticky="w")
    plot_ticks_var = tk.BooleanVar(value=True)
    tk.Checkbutton(zoom_window, variable=plot_ticks_var).grid(row=6, column=1)

    # Size and Position
    tk.Label(zoom_window, text="Inset Width (%):").grid(row=7, column=0, sticky="w")
    width_entry = tk.Entry(zoom_window)
    width_entry.insert(0, "30%")
    width_entry.grid(row=7, column=1)

    tk.Label(zoom_window, text="Inset Height (%):").grid(row=8, column=0, sticky="w")
    height_entry = tk.Entry(zoom_window)
    height_entry.insert(0, "30%")
    height_entry.grid(row=8, column=1)

    tk.Label(zoom_window, text="Inset Position:").grid(row=9, column=0, sticky="w")
    ...
    loc_menu.grid(row=9, column=1)

    tk.Label(zoom_window, text="Distance to plot edge:").grid(row=10, column=0, sticky="w")
    space_entry = tk.Entry(zoom_window)
    space_entry.insert(0, 1.5)
    space_entry.grid(row=10, column=1)

    tk.Button(zoom_window, text="Apply Zoom", command=apply_zoom).grid(row=11, column=0, columnspan=2, pady=10)
    tk.Button(zoom_window, text="Delete Selected", command=delete_selected_region).grid(row=11, column=2, columnspan=2, pady=10)

    tk.Label(zoom_window, text="Zoom Regions:").grid(row=12, column=0, columnspan=4, sticky="w")
    zoom_listbox = tk.Listbox(zoom_window, width=120)
    zoom_listbox.grid(row=13, column=0, columnspan=4, sticky="w")
    ...
    status_label.grid(row=14, column=0, columnspan=4, sticky="w", pady=(5, 0))
